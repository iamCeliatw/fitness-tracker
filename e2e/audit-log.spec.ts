...
test.describe("Audit Log — admin view", () => {
  ...
  test("audit log table renders filter buttons", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Appointment", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "WorkoutLog", exact: true }),
    ).toBeVisible();
  });

  test("non-admin user is redirected from /admin/audit-logs", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.waitForLoadState("networkidle");
  ...
  test("booking an appointment creates an audit log entry", async ({
    page,
    request,
  }) => {
    ...
    const beforeCount = totalText
      ? parseInt(totalText.match(/\d+/)?.[0] ?? "0")
      : 0;
