...
`POST /api/onboarding` SHALL 驗證登入身份後補完 membership：`mode=create`（orgName）建立 org 並成為 OWNER（含 inviteCode 撞碼重試與失敗補償刪除，與註冊一致）；`mode=join`（inviteCode）驗證邀請碼後以 MEMBER 加入。org 建立與邀請碼驗證邏輯 SHALL 與 `/api/auth/register` 共用（`src/lib/org.ts`），register 既有行為 MUST 不變。
