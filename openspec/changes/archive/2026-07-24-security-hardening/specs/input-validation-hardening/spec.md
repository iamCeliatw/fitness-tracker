...
`POST /api/auth/register` 的 `registerSchema` 中每個字串欄位 SHALL 有 `.max()` 上限：`name` ≤ 128、`orgName` ≤ 128、`password` ≤ 128、`inviteCode` 格式固定（長度與 regex 驗證）。超出上限 MUST 回傳 422。
...
`POST /api/onboarding` 的 `onboardingSchema` 中 `orgName` SHALL ≤ 128 字元，`inviteCode` SHALL 符合與 `registerSchema` 相同的格式規則。超出上限 MUST 回傳 422。
